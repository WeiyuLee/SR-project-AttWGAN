...
class evaluation:
    ...
    def load_model(self, config = {}, isNormallized=True):
        ...
        self.model_config['net'] = "Dis"
        self.model_config['d_inputs'] = predict_op
        self.model_config["d_model"] = "PatchWGAN_GP"
        dis_predict_op = mz.build_model(self.model_config)

        if type(predict_op) is tuple:
            self.predict_op = predict_op[0]
        else:
            #self.predict_op = predict_op
            self.predict_op = mz.inner_layer_results
    ...
    def prediction(self, predict_op, image):
     
        #run model in model_ticket_list and return prediction       
        predicted = self.sess.run(predict_op, feed_dict = {self.inputs:[image]})
        return predicted
    ...
    def run_evaluation(self):
        ...
        for k in self.predict_op:
            for img in grid_imgs:
                output_grids = {}
                self.viz_block = k
                for grid_key in grid_imgs[img]["grids"]:
                
                    pred = self.prediction(self.predict_op[k], grid_imgs[img]["grids"][grid_key])
                    output_grids[grid_key] = pred[0]
                #merged_img = self.merge_img(np.shape(output_grids[grid_key]), output_grids)
                merged_img = output_grids[grid_key]
                merged_img = (merged_img - np.min(merged_img))
                merged_img = merged_img/(np.max(merged_img) - np.min(merged_img)) * 255
                merged_img = np.round(merged_img, 0)
                merged_img = np.clip(merged_img, 0, 255).astype('uint8')
                if np.shape(merged_img)[2] == 1:
                    merged_img = np.repeat(merged_img, 3, axis=2)

                test_img = scipy.misc.toimage(merged_img, high=np.max(merged_img), low=np.min(merged_img))

                save_path = "./evaluation/test/"
                folder_name = img.split('/')[-1]
                folder_name = folder_name.split('/')[0]
                folder_name = os.path.join(save_path,folder_name)

                if not os.path.exists(folder_name):
                    os.makedirs(folder_name)
                save_path = os.path.join(folder_name, "{}_{}".format(self.viz_block,img.split('/')[-1]))
                test_img.save(save_path)
                #target_img.save("./evaluation/target/target_{}.png".format(progress))
                print("Process{}:{}/{}".format(k, progress, len(grid_imgs)))
                progress += 1
